...
=== SENSES (감지) ===
- measure(): 식물 특성 측정 (해바라기 꽃잎, 선인장 크기 등)
- get_entity_type(): 현재 위치의 엔티티 타입
- get_ground_type(): 현재 위치의 땅 타입
- can_harvest(): 수확 가능 여부
- get_pos_x(), get_pos_y(): 현재 드론 위치


=== NUM_UNLOCKED (언락 정보) ===
- num_unlocked(unlock): 특정 언락의 레벨 반환
- 예: num_unlocked(Unlocks.Speed) → 현재 속도 업그레이드 레벨
- 활용: 동적으로 맵 크기, 업그레이드 상태 확인 가능
- get_world_size(): 현재 맵 크기 (언락에 따라 변동)


=== WEIRD_SUBSTANCE (기묘한 물질) ===
- 감염된 식물 수확 시 50% 확률로 획득
- use_item(Items.Weird_Substance): 감염 상태 토글
  - 감염 식물 → 치료
  - 건강한 식물 → 감염
  - 인접 4방향 식물도 함께 토글됨!
...
주의사항:
- 인접 식물 토글 효과 때문에 패턴 고려 필요
- 감염된 식물 옆에 건강한 식물이 있으면:
  - 감염 식물은 치료되지만 건강한 식물은 감염됨
- 최적화: 모든 식물이 동일 상태일 때 사용하거나,
  - 개별적으로 치료할 때는 인접 식물 상태 확인 필요


=== DEBUG (디버그) ===
- print(): 값을 공중에 출력 + 출력 창에 기록
- quick_print(): 출력 창에만 기록 (더 빠름)
- 중단점: 코드 왼쪽 클릭으로 설정, 해당 줄에서 실행 멈춤
- 변수 위에 마우스 올리면 현재 값 표시
- 출력 창: 경고와 오류도 기록됨
- output.txt: 게임 폴더에 실행 로그 저장
"""
